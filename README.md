...
(`hypervisor/src/md/{mdgen,mdlint}`), ported from Sun's Solaris/Sun-Studio build
to a portable GNU make + gcc build. The C source is otherwise unmodified except
for a small set of portability fixes, warning cleanups and dead-code removals,
none of which change output: the rebuilt `mdgen` reproduces Sun's 2006 binaries
byte for byte.

**Every source change is recorded in [`CHANGELOG.md`](CHANGELOG.md)**, and each
touched file carries a `NOTICE:` comment stating what changed, immediately after
its original Sun header. The exception is `mdlint/basics.h`, which predates that
convention and carries only the little-endian `hton*/ntoh*` fix.

Current divergence from Sun's original — eight of the 26 files:

| Kind | Files |
|------|-------|
| Little-endian byte-swap | `mdgen/output_bin.c`, `mdlint/basics.h`, `mdlint/mdlint.c` |
| Missing `<string.h>` | `mdgen/mdmain.c`, `mdgen/mdparse.c`, `mdgen/output_dot.c`, `mdgen/output_bin.c` |
| Format / init / switch | `mdgen/mdmain.c`, `mdgen/output_dot.c`, `mdlint/mdlint.c` |
| Dead-code removal | `mdgen/mdparse.c`, `mdgen/output_text.c`, `mdgen/mdlex.l`, `mdlint/mdlint.c` |

Sun's SCCS `#pragma ident` stamps are **kept** (they record each file's version
and date at Sun); gcc does not implement that pragma, so the Makefile passes
`-Wno-unknown-pragmas` rather than removing the stamps.
...
(The list of files that differ from Sun's original, and why, is in Provenance
above; the full per-change history is in [`CHANGELOG.md`](CHANGELOG.md).)
